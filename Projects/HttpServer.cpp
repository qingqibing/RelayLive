...
namespace HttpWsServer
{
    ...
    int Init(void* uv)
    {
        ...
        info.port = port_value.empty()?80:stoi(port_value);
        info.protocols = protocols;
        info.mounts = &mount_web;
        info.vhost_name = "rtsp2http server";
        ...
        info.port = port_value.empty()?8000:stoi(port_value);
        info.protocols = wsprotocols;
        info.mounts = NULL;
        info.vhost_name = "rtsp2websocket server";
        if (!lws_create_vhost(context, &info)) {
